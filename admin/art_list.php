...
<?php
require('../include.php');
// $sql = "select * from em_article";
// $res = getAll($sql);
$sql = "select A.*,B.cate_name from em_article A, em_category B where A.cat_id = B.cate_id";
$res = getAll($sql);
// p($res);
// p($res);
// die;
...
<div class="main">
    <table cellpadding="0" cellspacing="0" class="ti">
        <tr>
            <th class="title" colspan="8">
                ...
                <span class="action-span"><a href="edite_art.php">添加文章</a></span>
            </th>
	...
	<?php if(!empty($res)) {?>
		<?php foreach($res as $val) {?>
	   			 ...
	   			 <td><?php echo $val['art_author']?></td>
                 <td><img style="width: 50px; height: 50px" src="<?php echo 'upload/'.$val['art_img']?>"></td>
                <!--是否显示-->
                 <?php if($val['art_open'] == 1) {?>
                    <td class="art_open"><img src="images/no.gif"/></td>
                <?php }?>
                <?php if($val['art_open'] == 0) {?>
                    <td class="art_open"><img src="images/yes.gif"/></td>
                <?php }?>   
                 ...
                 <td><?php echo date("Y-m-d H:i:s", $val['add_time'])?></td>
           			 ...
           			 <a href="">分配权限</a>　
	   			 <span style="margin-left:10px;"><a href="edite_art.php?artid=<?php echo $val['art_id']?>" id="mod">修改</a></span>
	   			 <span style="margin-left:20px;"><a href="action.php?act=delart&artid=<?php echo $val['art_id']?>">删除</a></span>
         			 </td>
	 		 </tr>
		  <?php }?>
	<?php }?>
